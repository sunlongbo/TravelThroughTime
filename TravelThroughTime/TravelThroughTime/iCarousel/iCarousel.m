...
#define MIN_TOGGLE_DURATION 0.2             // 最小toggle时间
#define MAX_TOGGLE_DURATION 0.4             // 最大toggle时间
#define SCROLL_DURATION 0.4                 // 滑动时间
#define INSERT_DURATION 0.4                 // 插入元素时间
#define DECELERATE_THRESHOLD 0.1            // 减速阈值
#define SCROLL_SPEED_THRESHOLD 2.0          // 滑动速度阈值
#define SCROLL_DISTANCE_THRESHOLD 0.1       // 滑动距离阈值
#define DECELERATION_MULTIPLIER 30.0        // 减速乘法因子
#define FLOAT_ERROR_MARGIN 0.000001         // 浮点误差
...
- (void)transformItemView:(UIView *)view atIndex:(NSInteger)index
{
    ...
    //special-case logic for iCarouselTypeCoverFlow2        toggle属性是专门给iCarouselTypeCoverFlow这个用的
    CGFloat clampedOffset = MAX(-1.0, MIN(1.0, offset));
...
- (void)step
{
    ...
    if (_scrolling && !_dragging)
    {
        ...
        if (time >= 1.0)
        {
            ...
            [_delegate carouselDidEndScrollingAnimation:self];      // 滑动结束代理
            [self popAnimationState];
        }
    }
    else if (_decelerating)
    {
        ...
        if (fabs(time - _scrollDuration) < FLOAT_ERROR_MARGIN)       //
        {
            ...
            {
                // _scrollToItemBoundary==NO &&
                // toggle状态是什么意思？
                CGFloat difference = round(_scrollOffset) - _scrollOffset;      // 【-0.5，0.5】
                if (difference > 0.5)
                {
                    // 这两个条件语句会被执行？
...
- (void)didTap:(UITapGestureRecognizer *)tapGesture
{
    ...
    UIView *itemView = [self itemViewAtPoint:[tapGesture locationInView:_contentView]];         // 获取被点击的view
    ...
    if (_scrollEnabled && _numberOfItems)
    {
        switch (panGesture.state)
        {
            ...
            case UIGestureRecognizerStateChanged:
            {
                ...
                CGFloat velocity = _vertical? [panGesture velocityInView:self].y: [panGesture velocityInView:self].x;   // 手势的滑动速度
                ...
                if (!_wrapEnabled && _bounces)
                {
                    ...
                    factor = 1.0 - MIN(fabs(_scrollOffset - [self clampedOffset:_scrollOffset]),
                                       _bounceDistance) / _bounceDistance;      //  [0, 1]
                }
